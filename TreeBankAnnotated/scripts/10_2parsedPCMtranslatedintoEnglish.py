...
with open('/local/musaeed/NaijaDiscourseClassification/TreeBankAnnotated/parsedDataDiscopy/translatedPCMParseFile.json', 'r') as file:
    data = json.load(file)
...
for item in data:
    item_data = json.loads(item['translatedDetails'])  # Parse item_data as a dictionary
    text = item_data['text']
...
df.to_csv('/local/musaeed/NaijaDiscourseClassification/TreeBankAnnotated/csv/processed/TreebankTanslatedPCMParsedToCsv.csv', index=False)
